...
- 完了　バリデーション未実装なので実装する。<br>
- 完了　商品購入後の価格の項目をsold表示にする。itemsテーブルのrimeainを使用。<br>
- 完了　JSON型を使用して一つのカラムに複数選択可能時の、保存、表示の実装の仕方を考える。<br>
- 完了　検索を絞ったままマイリストに移動できるように実装する<br>
- Laravelの画像編集ライブラリー”Intervention Image”の使用を検討しています。<br>
- とりあえず完了　汎用性の高い決済機能、stripeを使用しての開発を考える。<br>
...
> # 確認事項（模擬案件の時だけ掲載）<br>
- ユーザー認証後のトップページの検索機能についてですが、全体の検索機能の実装とは別に、マイリストページに移動した後に検索を実行するマイリスト検索機能の実装ではダメなのでしょうか？<br>
- 
<br>
<br>
<br>
...
# 修正履歴（模擬案件の時だけ掲載）<br>
- バリデーションの実装完了| ユーザ画像の初期画像の設置<br>
- 商品購入後の価格をsold表示をすると共に購入できないようにバリデーションを実装しました。<br>
- JSON型を使用してカテゴリーに関する送信、保存、表示、選択など実装完了<br>
- マイリストページ内の検索機能の実装<br>
- とりあえずstripeを使用して決済処理機能実装しました。<br>
