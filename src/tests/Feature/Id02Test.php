...
class Id02Test extends TestCase
{
    ...
    //ID2-3(2)ログイン時パスワードは合っているがアドレスが間違っている時
        $response = $this->post('/login', [
            'email' => 'test@example.com',
            'password' => 'pass1234',//パスワードが間違っていてアドレスが合っている場合
        ]);

        // バリデーションエラーによって302が返されることを確認
        $response->assertStatus(302);
        // emailフィールドに特定のメッセージがあることを確認
        $response->assertSessionHasErrors(['email' => 'ログイン情報が登録されていません。']);//fortityのシステムでpasswordのエラーもemailエラーと返しているためです。
